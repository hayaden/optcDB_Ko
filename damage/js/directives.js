    ...
    return {
        ...
        template: '<div id="hp"><div id="hpSlider"></div>' +
            '<div id="hp-rcv"><table><tbody>' +
                '<tr><td id="hp-cell" ng-click="setHP()">{{ hp.current | number }}</td><td>체력 ({{ hp.perc | number:2 }}%)</td></tr>' +
                '<tr><td>{{ numbers.rcv | number }}</td><td>회복력</td></tr>' +
                '<tr ng-show="numbers.healPerTurn"><td>{{ numbers.healPerTurn | number }}</td><td>회복/턴</td></tr>' +
                '<tr title="Minimum pirate level: {{ numbers.cost.level }}"><td>{{ numbers.cost.cost | number }}</td><td>코스트</td></tr>' +
            '</tbody></table></div>',
        link: function(scope, element, attrs) {
