...
/*******************
    ...
    function mapTypeToKo(typeValue) {
  if (!typeValue) return "";
  if (Array.isArray(typeValue)) {
    return typeValue.map(t => typeMap[t] || t).join("/");
  }
...
// filter by class
    ...
    var data = Object.values(window.units)
      ...
      .map(function (x, n) {
        var combinedType = Array.isArray(x.type) //맵핑작업을 위한 코드 추가
					? x.type.map(t => typeMap[t] || t).join("/")
					: typeMap[x.type] || x.type;
        var combinedClass = Array.isArray(x.class)
					? x.class
						.map(c =>
							Array.isArray(c)
        ...
        if (!x.type && x.id) {
          ...
          if (variant1 && variant1.type) {
            if (variant2 && variant2.type) {
              combinedType = mapTypeToKo([variant1.type, variant2.type]); // ✅ 여기!
              // 콤마 대신 / 로 보이게 하고 싶으면 위 함수 join("/") 그대로 사용
            } else {
              combinedType = mapTypeToKo(variant1.type); // ✅ 여기!
            }
